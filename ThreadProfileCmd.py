...
"Also, the tolerances might be different from what you wish to have.  I believe the internal minor diameters are all minimum and the external are all maximum.",\
        ...
        obj.addProperty("App::PropertyBool", "Advanced", "ThreadProfile", QT_TRANSLATE_NOOP("App::Property","Advanced thread profile"))
        obj.addProperty("App::PropertyFloat", "RootPitchRatio", "ThreadProfile", QT_TRANSLATE_NOOP("App::Property","Root to pitch length ratio"))
        obj.addProperty("App::PropertyFloat", "CrestPitchRatio", "ThreadProfile", QT_TRANSLATE_NOOP("App::Property","Crest to pitch length ratio"))
        obj.addProperty("App::PropertyAngle", "LeadAngle", "ThreadProfile", QT_TRANSLATE_NOOP("App::Property","Lead angle"))
        obj.addProperty("App::PropertyAngle", "TrailingAngle", "ThreadProfile", QT_TRANSLATE_NOOP("App::Property","Trailing angle"))
    ...
    def makePoints(self, obj):
        ...
        if not obj.Advanced:
            ...
            root_pitch_ratio = obj.RootPitchRatio
            crest_pitch_ratio = obj.CrestPitchRatio
            lead_angle = obj.LeadAngle
            trailing_angle = obj.TrailingAngle

            lead_pitch = (pitch * (1 - root_pitch_ratio - crest_pitch_ratio)) * math.tan(2 * math.pi * lead_angle / 360.0) / (math.tan(2 * math.pi * lead_angle / 360.0) + math.tan(2 * math.pi * trailing_angle / 360.0))
            trailing_pitch = pitch * (1 - root_pitch_ratio - crest_pitch_ratio) - lead_pitch

            height = lead_pitch / math.tan(2 * math.pi * lead_angle / 360.0)
            minor_radius = minor_diameter/2
            major_radius = minor_radius + height

            max_samples = 1440
            for ii in range(0, max_samples,step):
                alpha += math.pi * 2 / max_samples * step
                radius = self.advancedProfileCrossSectionRadius(alpha, minor_radius, major_radius, root_pitch_ratio, trailing_pitch/pitch, crest_pitch_ratio, lead_pitch/pitch)
                x = math.cos(alpha) * radius
                y = math.sin(alpha) * radius
                points.append(Base.Vector(x,y,0))
    ...
    def advancedProfileCrossSectionRadius(self, alpha, minor_diameter, major_diameter, root_pitch_ratio, trailing_pitch_ratio, crest_pitch_ratio, lead_pitch_ratio):
        alpha_ratio = alpha / (2 * math.pi)
        if (alpha_ratio < root_pitch_ratio / 2) or (alpha_ratio >= (1 - root_pitch_ratio / 2)):
            return minor_diameter
        elif alpha_ratio - root_pitch_ratio / 2 < lead_pitch_ratio:
            return minor_diameter + (alpha_ratio - root_pitch_ratio/2) * (major_diameter - minor_diameter) / lead_pitch_ratio
        elif alpha_ratio - root_pitch_ratio / 2 - lead_pitch_ratio < crest_pitch_ratio:
            return major_diameter
        elif alpha_ratio - root_pitch_ratio / 2 - lead_pitch_ratio - crest_pitch_ratio < trailing_pitch_ratio:
            return major_diameter + (alpha_ratio - root_pitch_ratio / 2 - lead_pitch_ratio - crest_pitch_ratio) * (minor_diameter - major_diameter) / trailing_pitch_ratio
        else:
            # this should never happen, but leaving it just in case
            return minor_diameter
                ...
                if preset_string in fp.preset_names:
                    ...
                    if idx != 0:
                        if not fp.Advanced:
                            ...
                            fp.Pitch = fp.presets_data[idx*7]
                            if "External" in fp.InternalOrExternal:
                                fp.MinorDiameter = fp.presets_data[idx*7+1]
                            else:
                                fp.MinorDiameter = fp.presets_data[idx*7+2]
                            fp.RootPitchRatio = fp.presets_data[idx*7+3]
                            fp.CrestPitchRatio = fp.presets_data[idx*7+4]
                            fp.LeadAngle = fp.presets_data[idx*7+5]
                            fp.TrailingAngle = fp.presets_data[idx*7+6]
    ...
    def execute(self, obj):
        ...
        if obj.Points:
            ...
            if obj.Closed and (len(obj.Points) > 2):
                ...
                spline.approximate(Points = obj.Points, DegMin = 3, DegMax = 5, Tolerance = .003692, Continuity = 'C1', ParamType = 'ChordLength')
                spline.setPeriodic()
...
class ThreadProfileCreateObjectCommandClass(object):
    ...
    def makeThreadProfile(self,name="VThreadProfile",minor_diameter=4.773,pitch=1,internal_or_external="External",internal_data=[],external_data=[],presets=[],thread_count=10,advanced=False,rootPitchRatio=0.25,crestPitchRatio=0.125,leadAngle=30,trailingAngle=30):
    ...
    that can be swept along a helix to produce a thread.  Code is based on Draft.makeBSpline()'''
        ...
        if not advanced:
            ...
            obj.RootPitchRatio=0.25
            obj.CrestPitchRatio=0.125
            obj.LeadAngle=30
            obj.TrailingAngle=30
...
class ThreadProfileCreateBottleObjectCommandClass(ThreadProfileCreateObjectCommandClass):
    ...
    def makeBottleThreadProfile(self):
        ...
        super(ThreadProfileCreateBottleObjectCommandClass, self).makeThreadProfile(name="Bottle_M_ThreadProfile",internal_data = internal_bottle_data, external_data = external_bottle_data, presets = bottle_presets_data,minor_diameter=bottle_presets_data[13][2],pitch=bottle_presets_data[13][1],internal_or_external="External",thread_count=3)

#Gui.addCommand("ThreadProfileCreateBottleObject", ThreadProfileCreateBottleObjectCommandClass())

####################################################################################
# Create the advanced thread profile object

class ThreadProfileCreateAdvancedObjectCommandClass(ThreadProfileCreateObjectCommandClass):
    """Create Object command"""

    def GetResources(self):
        return {'Pixmap'  : os.path.join( iconPath , 'CreateAdvancedObject.svg') ,
            'MenuText': "&Create Advanced thread profile" ,
            'ToolTip' : "Create an advanced thread ThreadProfile object"}

    def Activated(self):
        doc = FreeCAD.ActiveDocument
        doc.openTransaction("Create Advanced ThreadProfile")
        try:
            self.makeAdvancedThreadProfile()
        except:
            FreeCAD.Console.PrintError("ThreadProfile Error: Exception creating thread profile object.\n")
            QtGui.QApplication.restoreOverrideCursor()
        doc.commitTransaction()
    ...
    def makeAdvancedThreadProfile(self):
        ...
        def cd(txt, minor, major, tpi): #cd = calculate diameters
            ...
            internal = minor + offset
            return[txt, pitch, external, internal]
        advanced_presets_data = [
            ['Advanced V Thread Presets', 0,            0,           0,           0.25, 0.125, 30.0, 30.0],
            ['Garden Hose NHR',           0.08696*25.4, 0.9495*25.4, 0.9720*25.4, 0.25, 0.125, 30.0, 30.0],
            ['M1 Coarse',                 0.25,         0.693,       0.729,       0.25, 0.125, 30.0, 30.0],
            ['M1.1 Coarse',               0.25,         0.793,       0.829,       0.25, 0.125, 30.0, 30.0],
            ['M1.2 Coarse',               0.25,         0.893,       0.929,       0.25, 0.125, 30.0, 30.0],
            ['M1.4 Coarse',               0.30,         1.032,       1.075,       0.25, 0.125, 30.0, 30.0],
            ['M1.6 Coarse',               0.35,         1.171,       1.221,       0.25, 0.125, 30.0, 30.0],
            ['M1.8 Coarse',               0.35,         1.371,       1.421,       0.25, 0.125, 30.0, 30.0],
            ['M2 Coarse',                 0.40,         1.509,       1.567,       0.25, 0.125, 30.0, 30.0],
            ['M2.2 Coarse',               0.45,         1.648,       1.713,       0.25, 0.125, 30.0, 30.0],
            ['M2.5 Coarse',               0.45,         1.948,       2.013,       0.25, 0.125, 30.0, 30.0],
            ['M3 Coarse',                 0.50,         2.387,       2.459,       0.25, 0.125, 30.0, 30.0],
            ['M3.5 Coarse',               0.60,         2.764,       2.850,       0.25, 0.125, 30.0, 30.0],
            ['M4 Coarse',                 0.70,         3.141,       3.242,       0.25, 0.125, 30.0, 30.0],
            ['M4.5 Coarse',               0.75,         3.580,       3.688,       0.25, 0.125, 30.0, 30.0],
            ['M5 Coarse',                 0.80,         4.019,       4.134,       0.25, 0.125, 30.0, 30.0],
            ['M6 Coarse',                 1.00,         4.773,       4.917,       0.25, 0.125, 30.0, 30.0],
            ['M7 Coarse',                 1.00,         5.773,       5.917,       0.25, 0.125, 30.0, 30.0],
            ['M8 Coarse',                 1.25,         6.466,       6.647,       0.25, 0.125, 30.0, 30.0],
            ['M9 Coarse',                 1.25,         7.466,       7.647,       0.25, 0.125, 30.0, 30.0],
            ['M10 Coarse',                1.50,         8.160,       8.376,       0.25, 0.125, 30.0, 30.0],
            ['M11 Coarse',                1.50,         9.160,       9.376,       0.25, 0.125, 30.0, 30.0],
            ['M12 Coarse',                1.75,         9.853,       10.106,      0.25, 0.125, 30.0, 30.0],
            ['M14 Coarse',                2.00,         11.546,      11.835,      0.25, 0.125, 30.0, 30.0],
            ['M16 Coarse',                2.00,         13.546,      13.835,      0.25, 0.125, 30.0, 30.0],
            ['M18 Coarse',                2.50,         14.933,      15.394,      0.25, 0.125, 30.0, 30.0],
            ['M20 Coarse',                2.50,         16.933,      17.294,      0.25, 0.125, 30.0, 30.0],
            ['M22 Coarse',                2.50,         18.933,      19.294,      0.25, 0.125, 30.0, 30.0],
            ['M24 Coarse',                3.00,         20.319,      20.752,      0.25, 0.125, 30.0, 30.0],
            ['M27 Coarse',                3.00,         23.319,      23.752,      0.25, 0.125, 30.0, 30.0],
            ['M30 Coarse',                3.50,         25.706,      26.211,      0.25, 0.125, 30.0, 30.0],
            ['M33 Coarse',                3.50,         28.706,      29.211,      0.25, 0.125, 30.0, 30.0],
            ['M36 Coarse',                4.00,         31.093,      31.670,      0.25, 0.125, 30.0, 30.0],
            ['M39 Coarse',                4.00,         34.093,      34.670,      0.25, 0.125, 30.0, 30.0],
            ['M42 Coarse',                4.50,         36.479,      37.129,      0.25, 0.125, 30.0, 30.0],
            ['M45 Coarse',                4.50,         39.479,      40.129,      0.25, 0.125, 30.0, 30.0],
            ['M48 Coarse',                5.00,         41.866,      42.857,      0.25, 0.125, 30.0, 30.0],
            ['M52 Coarse',                5.00,         45.866,      46.587,      0.25, 0.125, 30.0, 30.0],
            ['M56 Coarse',                5.50,         49.252,      50.046,      0.25, 0.125, 30.0, 30.0],
            ['M60 Coarse',                5.50,         53.252,      54.046,      0.25, 0.125, 30.0, 30.0],
            ['M64 Coarse',                6.00,         56.639,      57.505,      0.25, 0.125, 30.0, 30.0],
            ['M68 Coarse',                6.00,         60.639,      61.505,      0.25, 0.125, 30.0, 30.0],
            ['M1 Fine',                   0.20,         0.755,       0.783,       0.25, 0.125, 30.0, 30.0],
            ['M1.1 Fine',                 0.20,         0.855,       0.883,       0.25, 0.125, 30.0, 30.0],
            ['M1.2 Fine',                 0.20,         0.955,       0.983,       0.25, 0.125, 30.0, 30.0],
            ['M1.4 Fine',                 0.20,         1.155,       1.183,       0.25, 0.125, 30.0, 30.0],
            ['M1.6 Fine',                 0.20,         1.355,       1.383,       0.25, 0.125, 30.0, 30.0],
            ['M1.8 Fine',                 0.20,         1.555,       1.583,       0.25, 0.125, 30.0, 30.0],
            ['M2 Fine',                   0.25,         1.693,       1.729,       0.25, 0.125, 30.0, 30.0],
            ['M2.2 Fine',                 0.25,         1.893,       1.929,       0.25, 0.125, 30.0, 30.0],
            ['M2.5 Fine',                 0.35,         2.071,       2.121,       0.25, 0.125, 30.0, 30.0],
            ['M3 Fine',                   0.35,         2.571,       2.621,       0.25, 0.125, 30.0, 30.0],
            ['M3.5 Fine',                 0.35,         3.071,       3.121,       0.25, 0.125, 30.0, 30.0],
            ['M4 Fine',                   0.50,         3.387,       3.459,       0.25, 0.125, 30.0, 30.0],
            ['M4.5 Fine',                 0.50,         3.887,       3.959,       0.25, 0.125, 30.0, 30.0],
            ['M5 Fine',                   0.50,         4.387,       4.459,       0.25, 0.125, 30.0, 30.0],
            ['M5.5 Fine',                 0.50,         4.887,       4.959,       0.25, 0.125, 30.0, 30.0],
            ['M6 Fine',                   0.75,         5.080,       5.188,       0.25, 0.125, 30.0, 30.0],
            ['M7 Fine',                   0.75,         6.080,       6.188,       0.25, 0.125, 30.0, 30.0],
            ['M8 Fine',                   0.75,         7.080,       7.188,       0.25, 0.125, 30.0, 30.0],
            ['M8 Fine',                   1.00,         6.773,       6.917,       0.25, 0.125, 30.0, 30.0],
            ['M9 Fine',                   0.75,         8.080,       8.188,       0.25, 0.125, 30.0, 30.0],
            ['M9 Fine',                   1.00,         7.773,       7.917,       0.25, 0.125, 30.0, 30.0],
            ['M10 Fine',                  0.75,         9.080,       9.188,       0.25, 0.125, 30.0, 30.0],
            ['M10 Fine',                  1.00,         8.773,       8.917,       0.25, 0.125, 30.0, 30.0],
            ['M10 Fine',                  1.25,         8.466,       8.647,       0.25, 0.125, 30.0, 30.0],
            ['M11 Fine',                  0.75,         10.080,      10.188,      0.25, 0.125, 30.0, 30.0],
            ['M11 Fine',                  1.00,         9.773,       9.917,       0.25, 0.125, 30.0, 30.0],
            ['M12 Fine',                  1.00,         10.773,      10.917,      0.25, 0.125, 30.0, 30.0],
            ['M12 Fine',                  1.25,         10.466,      10.647,      0.25, 0.125, 30.0, 30.0],
            ['M12 Fine',                  1.50,         10.160,      10.376,      0.25, 0.125, 30.0, 30.0],
            ['M14 Fine',                  1.00,         12.773,      12.917,      0.25, 0.125, 30.0, 30.0],
            ['M14 Fine',                  1.25,         12.466,      12.647,      0.25, 0.125, 30.0, 30.0],
            ['M14 Fine',                  1.50,         12.160,      12.376,      0.25, 0.125, 30.0, 30.0],
            ['M15 Fine',                  1.00,         13.773,      13.917,      0.25, 0.125, 30.0, 30.0],
            ['M15 Fine',                  1.50,         13.160,      13.376,      0.25, 0.125, 30.0, 30.0],
            ['M16 Fine',                  1.00,         14.773,      14.917,      0.25, 0.125, 30.0, 30.0],
            ['M16 Fine',                  1.50,         14.160,      14.376,      0.25, 0.125, 30.0, 30.0],
            ['M17 Fine',                  1.00,         15.773,      15.917,      0.25, 0.125, 30.0, 30.0],
            ['M17 Fine',                  1.50,         15.160,      15.376,      0.25, 0.125, 30.0, 30.0],
            ['M18 Fine',                  1.00,         16.773,      16.917,      0.25, 0.125, 30.0, 30.0],
            ['M18 Fine',                  1.50,         16.160,      16.376,      0.25, 0.125, 30.0, 30.0],
            ['M18 Fine',                  2.00,         15.546,      15.835,      0.25, 0.125, 30.0, 30.0],
            ['M20 Fine',                  1.00,         18.773,      18.917,      0.25, 0.125, 30.0, 30.0],
            ['M20 Fine',                  1.50,         18.160,      18.376,      0.25, 0.125, 30.0, 30.0],
            ['M20 Fine',                  2.00,         17.546,      17.835,      0.25, 0.125, 30.0, 30.0],
            ['M22 Fine',                  1.00,         20.773,      20.917,      0.25, 0.125, 30.0, 30.0],
            ['M22 Fine',                  1.50,         20.160,      20.376,      0.25, 0.125, 30.0, 30.0],
            ['M22 Fine',                  2.00,         19.546,      19.835,      0.25, 0.125, 30.0, 30.0],
            ['M24 Fine',                  1.00,         22.773,      22.917,      0.25, 0.125, 30.0, 30.0],
            ['M24 Fine',                  1.50,         22.160,      22.376,      0.25, 0.125, 30.0, 30.0],
            ['M24 Fine',                  2.00,         21.546,      21.835,      0.25, 0.125, 30.0, 30.0],
            ['M25 Fine',                  1.00,         23.773,      23.917,      0.25, 0.125, 30.0, 30.0],
            ['M25 Fine',                  1.50,         23.160,      23.376,      0.25, 0.125, 30.0, 30.0],
            ['M25 Fine',                  2.00,         22.546,      22.835,      0.25, 0.125, 30.0, 30.0],
            ['M27 Fine',                  1.00,         25.773,      25.917,      0.25, 0.125, 30.0, 30.0],
            ['M27 Fine',                  1.50,         25.160,      25.376,      0.25, 0.125, 30.0, 30.0],
            ['M27 Fine',                  2.00,         24.546,      24.835,      0.25, 0.125, 30.0, 30.0],
            ['M28 Fine',                  1.00,         26.773,      26.917,      0.25, 0.125, 30.0, 30.0],
            ['M28 Fine',                  1.50,         26.160,      26.376,      0.25, 0.125, 30.0, 30.0],
            ['M28 Fine',                  2.00,         25.546,      25.835,      0.25, 0.125, 30.0, 30.0],
            ['M30 Fine',                  1.00,         28.773,      28.917,      0.25, 0.125, 30.0, 30.0],
            ['M30 Fine',                  1.50,         28.160,      28.376,      0.25, 0.125, 30.0, 30.0],
            ['M30 Fine',                  2.00,         27.546,      27.835,      0.25, 0.125, 30.0, 30.0],
            ['M30 Fine',                  3.00,         26.319,      26.752,      0.25, 0.125, 30.0, 30.0],
            ['M32 Fine',                  1.50,         30.160,      30.376,      0.25, 0.125, 30.0, 30.0],
            ['M32 Fine',                  2.00,         29.546,      29.835,      0.25, 0.125, 30.0, 30.0],
            ['M33 Fine',                  1.50,         31.160,      31.376,      0.25, 0.125, 30.0, 30.0],
            ['M33 Fine',                  2.00,         30.546,      30.835,      0.25, 0.125, 30.0, 30.0],
            ['M33 Fine',                  3.00,         29.319,      29.752,      0.25, 0.125, 30.0, 30.0],
            ['M35 Fine',                  1.50,         33.160,      33.376,      0.25, 0.125, 30.0, 30.0],
            ['M35 Fine',                  2.00,         32.546,      32.835,      0.25, 0.125, 30.0, 30.0],
            ['M36 Fine',                  1.50,         34.160,      34.376,      0.25, 0.125, 30.0, 30.0],
            ['M36 Fine',                  2.00,         33.546,      33.835,      0.25, 0.125, 30.0, 30.0],
            ['M36 Fine',                  3.00,         32.319,      32.752,      0.25, 0.125, 30.0, 30.0],
            ['M39 Fine',                  1.50,         37.160,      37.376,      0.25, 0.125, 30.0, 30.0],
            ['M39 Fine',                  2.00,         36.546,      36.835,      0.25, 0.125, 30.0, 30.0],
            ['M39 Fine',                  3.00,         35.319,      35.752,      0.25, 0.125, 30.0, 30.0],
            ['M40 Fine',                  1.50,         38.160,      38.376,      0.25, 0.125, 30.0, 30.0],
            ['M40 Fine',                  2.00,         37.546,      37.835,      0.25, 0.125, 30.0, 30.0],
            ['M40 Fine',                  3.00,         36.619,      36.752,      0.25, 0.125, 30.0, 30.0],
            ['M42 Fine',                  1.50,         40.160,      40.376,      0.25, 0.125, 30.0, 30.0],
            ['M42 Fine',                  2.00,         39.546,      39.835,      0.25, 0.125, 30.0, 30.0],
            ['M42 Fine',                  3.00,         38.319,      38.752,      0.25, 0.125, 30.0, 30.0],
            ['M42 Fine',                  4.00,         37.093,      37.670,      0.25, 0.125, 30.0, 30.0],
            ['M45 Fine',                  1.50,         43.160,      43.376,      0.25, 0.125, 30.0, 30.0],
            ['M45 Fine',                  2.00,         42.546,      42.835,      0.25, 0.125, 30.0, 30.0],
            ['M45 Fine',                  3.00,         41.319,      41.752,      0.25, 0.125, 30.0, 30.0],
            ['M45 Fine',                  4.00,         40.093,      40.670,      0.25, 0.125, 30.0, 30.0],
            ['M48 Fine',                  1.50,         46.160,      46.376,      0.25, 0.125, 30.0, 30.0],
            ['M48 Fine',                  2.00,         45.546,      45.835,      0.25, 0.125, 30.0, 30.0],
            ['M48 Fine',                  3.00,         44.319,      44.752,      0.25, 0.125, 30.0, 30.0],
            ['M48 Fine',                  4.00,         43.093,      43.670,      0.25, 0.125, 30.0, 30.0],
            ['M50 Fine',                  1.50,         48.160,      48.376,      0.25, 0.125, 30.0, 30.0],
            ['M50 Fine',                  2.00,         47.546,      47.835,      0.25, 0.125, 30.0, 30.0],
            ['M50 Fine',                  3.00,         46.319,      46.752,      0.25, 0.125, 30.0, 30.0],
            ['M52 Fine',                  1.50,         50.160,      50.376,      0.25, 0.125, 30.0, 30.0],
            ['M52 Fine',                  2.00,         49.546,      49.835,      0.25, 0.125, 30.0, 30.0],
            ['M52 Fine',                  3.00,         48.319,      48.752,      0.25, 0.125, 30.0, 30.0],
            ['M52 Fine',                  4.00,         47.093,      47.670,      0.25, 0.125, 30.0, 30.0],
            ['M55 Fine',                  1.50,         53.160,      53.376,      0.25, 0.125, 30.0, 30.0],
            ['M55 Fine',                  2.00,         52.546,      52.835,      0.25, 0.125, 30.0, 30.0],
            ['M55 Fine',                  3.00,         51.319,      51.752,      0.25, 0.125, 30.0, 30.0],
            ['M55 Fine',                  4.00,         50.093,      50.670,      0.25, 0.125, 30.0, 30.0],
            ['M56 Fine',                  1.50,         54.160,      54.376,      0.25, 0.125, 30.0, 30.0],
            ['M56 Fine',                  2.00,         43.546,      53.835,      0.25, 0.125, 30.0, 30.0],
            ['M56 Fine',                  3.00,         52.319,      52.752,      0.25, 0.125, 30.0, 30.0],
            ['M56 Fine',                  4.00,         51.903,      51.670,      0.25, 0.125, 30.0, 30.0],
            ['M58 Fine',                  1.50,         56.160,      56.376,      0.25, 0.125, 30.0, 30.0],
            ['M58 Fine',                  2.00,         55.546,      55.835,      0.25, 0.125, 30.0, 30.0],
            ['M58 Fine',                  3.00,         54.319,      54.752,      0.25, 0.125, 30.0, 30.0],
            ['M58 Fine',                  4.00,         53.093,      53.670,      0.25, 0.125, 30.0, 30.0],
            ['M60 Fine',                  1.50,         58.160,      58.376,      0.25, 0.125, 30.0, 30.0],
            ['M60 Fine',                  2.00,         57.546,      57.835,      0.25, 0.125, 30.0, 30.0],
            ['M60 Fine',                  3.00,         56.319,      56.752,      0.25, 0.125, 30.0, 30.0],
            ['M60 Fine',                  4.00,         55.093,      55.670,      0.25, 0.125, 30.0, 30.0],
            ['M62 Fine',                  1.50,         60.160,      60.376,      0.25, 0.125, 30.0, 30.0],
            ['M62 Fine',                  2.00,         59.546,      59.835,      0.25, 0.125, 30.0, 30.0],
            ['M62 Fine',                  3.00,         58.319,      58.752,      0.25, 0.125, 30.0, 30.0],
            ['M62 Fine',                  4.00,         57.093,      57.670,      0.25, 0.125, 30.0, 30.0],
            ['M64 Fine',                  1.50,         62.160,      62.376,      0.25, 0.125, 30.0, 30.0],
            ['M64 Fine',                  2.00,         61.546,      61.835,      0.25, 0.125, 30.0, 30.0],
            ['M64 Fine',                  3.00,         60.319,      60.752,      0.25, 0.125, 30.0, 30.0],
            ['M64 Fine',                  4.00,         59.093,      59.670,      0.25, 0.125, 30.0, 30.0],
            ['M65 Fine',                  1.50,         63.160,      63.376,      0.25, 0.125, 30.0, 30.0],
            ['M65 Fine',                  2.00,         62.546,      62.835,      0.25, 0.125, 30.0, 30.0],
            ['M65 Fine',                  3.00,         61.319,      61.752,      0.25, 0.125, 30.0, 30.0],
            ['M65 Fine',                  4.00,         60.093,      60.670,      0.25, 0.125, 30.0, 30.0],
            ['M68 Fine',                  1.50,         66.160,      66.376,      0.25, 0.125, 30.0, 30.0],
            ['M68 Fine',                  2.00,         65.546,      65.835,      0.25, 0.125, 30.0, 30.0],
            ['M68 Fine',                  3.00,         64.319,      64.752,      0.25, 0.125, 30.0, 30.0],
            ['M68 Fine',                  4.00,         63.093,      63.670,      0.25, 0.125, 30.0, 30.0],
            ['M70 Fine',                  1.50,         68.160,      68.376,      0.25, 0.125, 30.0, 30.0],
            ['M70 Fine',                  2.00,         67.546,      67.835,      0.25, 0.125, 30.0, 30.0],
            ['M70 Fine',                  3.00,         66.319,      66.752,      0.25, 0.125, 30.0, 30.0],
            ['M70 Fine',                  4.00,         65.093,      65.670,      0.25, 0.125, 30.0, 30.0],
            ['M70 Fine',                  6.00,         62.639,      63.505,      0.25, 0.125, 30.0, 30.0],
            ['M72 Fine',                  1.50,         70.160,      70.376,      0.25, 0.125, 30.0, 30.0],
            ['M72 Fine',                  2.00,         69.546,      69.835,      0.25, 0.125, 30.0, 30.0],
            ['M72 Fine',                  3.00,         68.319,      68.752,      0.25, 0.125, 30.0, 30.0],
            ['M72 Fine',                  4.00,         67.093,      67.670,      0.25, 0.125, 30.0, 30.0],
            ['M72 Fine',                  6.00,         64.639,      65.505,      0.25, 0.125, 30.0, 30.0],
            ['M75 Fine',                  1.50,         73.160,      73.376,      0.25, 0.125, 30.0, 30.0],
            ['M75 Fine',                  2.00,         72.546,      72.835,      0.25, 0.125, 30.0, 30.0],
            ['M75 Fine',                  3.00,         71.319,      71.752,      0.25, 0.125, 30.0, 30.0],
            ['M75 Fine',                  4.00,         70.093,      70.670,      0.25, 0.125, 30.0, 30.0],
            ['M75 Fine',                  6.00,         67.639,      68.505,      0.25, 0.125, 30.0, 30.0],
            ['M76 Fine',                  1.50,         74.160,      74.376,      0.25, 0.125, 30.0, 30.0],
            ['M76 Fine',                  2.00,         73.546,      73.835,      0.25, 0.125, 30.0, 30.0],
            ['M76 Fine',                  3.00,         72.319,      72.752,      0.25, 0.125, 30.0, 30.0],
            ['M76 Fine',                  4.00,         71.093,      71.670,      0.25, 0.125, 30.0, 30.0],
            ['M76 Fine',                  6.00,         68.639,      69.505,      0.25, 0.125, 30.0, 30.0],
            ['M80 Fine',                  1.50,         78.160,      78.376,      0.25, 0.125, 30.0, 30.0],
            ['M80 Fine',                  2.00,         77.546,      77.835,      0.25, 0.125, 30.0, 30.0],
            ['M80 Fine',                  3.00,         76.319,      76.752,      0.25, 0.125, 30.0, 30.0],
            ['M80 Fine',                  4.00,         75.093,      75.670,      0.25, 0.125, 30.0, 30.0],
            ['M80 Fine',                  6.00,         72.639,      73.505,      0.25, 0.125, 30.0, 30.0],
            ['M85 Fine',                  2.00,         82.546,      82.535,      0.25, 0.125, 30.0, 30.0],
            ['M85 Fine',                  3.00,         81.319,      81.752,      0.25, 0.125, 30.0, 30.0],
            ['M85 Fine',                  4.00,         80.093,      80.670,      0.25, 0.125, 30.0, 30.0],
            ['M85 Fine',                  6.00,         77.639,      78.505,      0.25, 0.125, 30.0, 30.0],
            ['M90 Fine',                  2.00,         87.546,      87.835,      0.25, 0.125, 30.0, 30.0],
            ['M90 Fine',                  3.00,         86.319,      86.752,      0.25, 0.125, 30.0, 30.0],
            ['M90 Fine',                  4.00,         85.093,      85.670,      0.25, 0.125, 30.0, 30.0],
            ['M90 Fine',                  6.00,         82.639,      83.505,      0.25, 0.125, 30.0, 30.0],
            ['M95 Fine',                  2.00,         92.546,      92.835,      0.25, 0.125, 30.0, 30.0],
            ['M95 Fine',                  3.00,         91.319,      91.752,      0.25, 0.125, 30.0, 30.0],
            ['M95 Fine',                  4.00,         90.093,      90.670,      0.25, 0.125, 30.0, 30.0],
            ['M95 Fine',                  6.00,         87.639,      88.505,      0.25, 0.125, 30.0, 30.0],
            ['M100 Fine',                 2.00,         97.546,      97.835,      0.25, 0.125, 30.0, 30.0],
            ['M100 Fine',                 3.00,         96.319,      96.752,      0.25, 0.125, 30.0, 30.0],
            ['M100 Fine',                 4.00,         95.093,      95.670,      0.25, 0.125, 30.0, 30.0],
            ['M100 Fine',                 6.00,         92.639,      93.505,      0.25, 0.125, 30.0, 30.0],
            ['1/4 in-20 UNC',             25.4*0.0500,  25.4*0.1887, 25.4*0.1959, 0.25, 0.125, 30.0, 30.0],
            ['5/16 in-18 UNC UNC',        25.4*0.0556,  25.4*0.2443, 25.4*0.2524, 0.25, 0.125, 30.0, 30.0],
            ['3/8 in-16 UNC UNC',         25.4*0.0625,  25.4*0.2983, 25.4*0.3073, 0.25, 0.125, 30.0, 30.0],
            ['7/16 in-14 UNC',            25.4*0.0714,  25.4*0.3499, 25.4*0.3602, 0.25, 0.125, 30.0, 30.0],
            ['1/2 in-13 UNC',             25.4*0.0769,  25.4*0.4056, 25.4*0.4167, 0.25, 0.125, 30.0, 30.0],
            ['9/16 in-12 UNC',            25.4*0.0833,  25.4*0.4603, 25.4*0.4723, 0.25, 0.125, 30.0, 30.0],
            ['5/8 in-11 UNC',             25.4*0.0909,  25.4*0.5135, 25.4*0.5266, 0.25, 0.125, 30.0, 30.0],
            ['3/4 in-10 UNC',             25.4*0.1000,  25.4*0.6273, 25.4*0.6417, 0.25, 0.125, 30.0, 30.0],
            ['7/8 in-9 UNC',              25.4*0.1111,  25.4*0.7387, 25.4*0.7547, 0.25, 0.125, 30.0, 30.0],
            ['1 in-8 UNC',                25.4*0.1250,  25.4*0.8466, 25.4*0.8647, 0.25, 0.125, 30.0, 30.0],
            ['1 1/8 in-7 UNC',            25.4*0.1429,  25.4*0.9497, 25.4*0.9704, 0.25, 0.125, 30.0, 30.0],
            ['1 1/4 in-7 UNC',            25.4*0.1429,  25.4*1.0747, 25.4*1.0954, 0.25, 0.125, 30.0, 30.0],
            ['1 3/8 in-6 UNC',            25.4*0.1667,  25.4*1.1705, 25.4*1.1946, 0.25, 0.125, 30.0, 30.0],
            ['1 1/2 in-6 UNC',            25.4*0.1667,  25.4*1.2955, 25.4*1.3196, 0.25, 0.125, 30.0, 30.0],
            ['1 3/4 in-5 UNC',            25.4*0.2000,  25.4*1.5046, 25.4*1.5335, 0.25, 0.125, 30.0, 30.0],
            ['2 in-4 1/2 UNC',            25.4*0.2222,  25.4*1.7274, 25.4*1.7594, 0.25, 0.125, 30.0, 30.0],
            ['2 1/4 in-4 1/2 UNC',        25.4*0.2222,  25.4*1.9774, 25.4*2.0094, 0.25, 0.125, 30.0, 30.0],
            ['2-1/2 in-4 UNC',            25.4*0.2500,  25.4*2.1992, 25.4*2.229,  0.25, 0.125, 30.0, 30.0],
            ['2-3/4 in-4 UNC',            25.4*0.2500,  25.4*2.4491, 25.4*2.479,  0.25, 0.125, 30.0, 30.0],
            ['3 in-4 UNC',                25.4*0.2500,  25.4*2.699,  25.4*2.729,  0.25, 0.125, 30.0, 30.0],
            ['3-1/4 in-4 UNC',            25.4*0.2500,  25.4*2.949,  25.4*2.979,  0.25, 0.125, 30.0, 30.0],
            ['3-1/2 in-4 UNC',            25.4*0.2500,  25.4*3.199,  25.4*3.229,  0.25, 0.125, 30.0, 30.0],
            ['3-3/4 in-4 UNC',            25.4*0.2500,  25.4*3.4489, 25.4*3.479,  0.25, 0.125, 30.0, 30.0],
            ['4 in-4 UNC',                25.4*0.2500,  25.4*3.6989, 25.4*3.729,  0.25, 0.125, 30.0, 30.0],
            ['1/4 in-28 UNF UNF',         25.4*0.0357,  25.4*0.2062, 25.4*0.2113, 0.25, 0.125, 30.0, 30.0],
            ['5/16 in-24 UNF',            25.4*0.0417,  25.4*0.2614, 25.4*0.2674, 0.25, 0.125, 30.0, 30.0],
            ['3/8 in-24 UNF',             25.4*0.0417,  25.4*0.3239, 25.4*0.3299, 0.25, 0.125, 30.0, 30.0],
            ['7/16 in-20 UNF',            25.4*0.0500,  25.4*0.3762, 25.4*0.3834, 0.25, 0.125, 30.0, 30.0],
            ['1/2 in-20 UNF',             25.4*0.0500,  25.4*0.4387, 25.4*0.4459, 0.25, 0.125, 30.0, 30.0],
            ['9/16 in-18 UNF',            25.4*0.0556,  25.4*0.4943, 25.4*0.5024, 0.25, 0.125, 30.0, 30.0],
            ['5/8 in-18 UNF',             25.4*0.0556,  25.4*0.5568, 25.4*0.5649, 0.25, 0.125, 30.0, 30.0],
            ['3/4 in-16 UNF',             25.4*0.0625,  25.4*0.6733, 25.4*0.6823, 0.25, 0.125, 30.0, 30.0],
            ['7/8 in-14 UNF',             25.4*0.0714,  25.4*0.7874, 25.4*0.7977, 0.25, 0.125, 30.0, 30.0],
            ['1 in-12 UNF',               25.4*0.0714,  25.4*0.8978, 25.4*0.9098, 0.25, 0.125, 30.0, 30.0],
            ['1 in-14 UNF',               25.4*0.0714,  25.4*0.9132, 25.4*0.923,  0.25, 0.125, 30.0, 30.0],
            ['1 1/4 in-12 UNF',           25.4*0.0833,  25.4*1.0228, 25.4*1.0348, 0.25, 0.125, 30.0, 30.0],
            ['1 1/8 in-12 UNF',           25.4*0.0833,  25.4*1.1478, 25.4*1.1598, 0.25, 0.125, 30.0, 30.0],
            ['1 3/8 in-12 UNF',           25.4*0.0833,  25.4*1.2728, 25.4*1.2848, 0.25, 0.125, 30.0, 30.0],
            ['1 1/2 in-12 UNF',           25.4*0.0833,  25.4*1.3978, 25.4*1.4098, 0.25, 0.125, 30.0, 30.0],
            ]

        default_profile_index = 13

        super(ThreadProfileCreateAdvancedObjectCommandClass, self).makeThreadProfile(name="Advanced_M_ThreadProfile",internal_data = [], external_data = [], presets = advanced_presets_data,minor_diameter=advanced_presets_data[default_profile_index][2],pitch=advanced_presets_data[default_profile_index][1],internal_or_external="External",thread_count=10,advanced=True,rootPitchRatio=advanced_presets_data[default_profile_index][4],crestPitchRatio=advanced_presets_data[default_profile_index][5],leadAngle=advanced_presets_data[default_profile_index][6],trailingAngle=advanced_presets_data[default_profile_index][7])

#Gui.addCommand("ThreadProfileCreateAdvancedObject", ThreadProfileCreateAdvancedObjectCommandClass())
